...
export const timeAsDigitalClock = milliseconds => {
  const totalSeconds = Math.floor(milliseconds / 1000)
  const roundedMinutes = Math.floor(totalSeconds / 60)
  const extraSeconds = totalSeconds - (roundedMinutes * 60)
  const add0 = n => (n < 10 ? '0' : '') + n
  return `${add0(roundedMinutes)}:${add0(extraSeconds)}`
}

export const trimArray = ({ array, from = 0, until, count }) => {
  if (until === Infinity){
